...
namespace QProSapAddIn
{
    internal class TableViewerPanelViewModel : DockPane, INotifyPropertyChanged
    {
            ...
            try {
                ...
                OracleCommand cmd = new OracleCommand("select COLUMN_NAME,DATA_TYPE from SYS.ALL_TAB_COLUMNS where OWNER like '" + _currentSchema + "' and table_name = '" + _currentTable + "'" + " AND rownum <= 50", Globals.DBConn);
                        ...
                        if (!dr.IsDBNull(0) && dr.GetString(0).IndexOfAny(new char[] { ' ', '/', '\\' }) == -1)  //if there is a slash in the column namestring then skip it
                    {
                        ...
                        else if (dr.GetString(0) == "OBJECTID")
                            {
                        ...
                        else if (dr.GetString(1) == "FLOAT")   //if value is too big for arcgis.  It must be NUMBER(38,8)
                        {
                            ...
                            colls.Add(" CAST(" + dr.GetString(0) + " AS NUMBER(38,0)) ");
                            }
                            else
                            {
                                colls.Add(dr.GetString(0));
                ...
                var result = string.Join(",", colls.ToArray());
                    ...
                    string searchFor = _spatialCol.SelectString;    //make sure it is exactly same field ,SHAPE,
                                                                    //hana string replaceWith = "ST_AsWKT(" + _spatialCol.SelectString + ") ";
                    //this is for hana only
                    //string replaceWith = "ST_ASTEXT(" + _spatialCol.SelectString + ")";
                    //_querytext.SelectString = System.Text.RegularExpressions.Regex.Replace(_querytext.SelectString, searchFor, replaceWith);
        ...
        public async Task ExecuteSqlCallback(CancelableProgressorSource cpd)
        {
            try
            {
                await QueuedTask.Run(() =>
                {
                    ...
                    if (Globals.RdbmsType == "Oracle")
                    {
                        ...
                        OracleCommand cmd = Globals.DBConn.CreateCommand();

                        queryStr = "SELECT SDE.ST_ASTEXT(SHAPE),OBJECTID   FROM  SDE.SEWER_PIPES";
                        ...
                        ////oracledatareader is not converting st_astext() function properly, so had to build explicit string
                        ...
                        //    {
                        //        //get num fields
                        //        var objCol = new object[dataReader.FieldCount];
                        //        while (colCounter < objCol.Length)   //loops thru fields
                        //        {
                        //            //add column to table
                        //            DataColumn dc = new DataColumn(dataReader.GetName(colCounter));
                        ...
                        //            object[] newRow = new object[dataReader.FieldCount];

                        //            // Set the values of the array.
                        //            //newRow[0] = "Hello";
                        //            //newRow[1] = "World";
                        //            //newRow[2] = "two";
                        //            while (colCounter < objCol.Length)
                        //            {
                        //                //get the data values
                        //                newRow[colCounter] = dataReader.GetValue(colCounter);
                        //                //newRow[colCounter] = dataReader.GetValue(colCounter) is DBNull ? string.Empty : dataReader.GetValue(colCounter);                                   //newRow[colCounter]= objCol.GetValue[colCounter];
                        //                colCounter = colCounter + 1;
                        //            }
                        ...
                        //            row = dataTable.LoadDataRow(newRow, true);

                        //            rowCounter = rowCounter + 1;

                        //        }
                        //        dataTable.EndLoadData();
                        //        _actrecCount.SelectString = rowCounter.ToString();
                        //        dataReader.Close();
                        //        dataReader.Dispose();
                        //    }
                        //    else
                        ...
                        using (OracleDataReader reader = cmd.ExecuteReader())
                        {
                            //create table schema
                            int fldCnt = 0;
                            while (fldCnt < reader.FieldCount)
                            {
                                DataColumn col = new DataColumn(reader.GetName(fldCnt));

                                //handle SHAPE column name
                                ...
                                if (end != -1)
                                {
                                    col.ColumnName = _spatialCol.SelectString; //rename column 
                                }
                        ...
                        {
                            int rowCntr = 0;
                            totFldCnt = reader.FieldCount;

                            // Create an array for the values.
                            object[] newRow = new object[totFldCnt];
                            ...
                            //newRow[2] = "two";
                            dtab.BeginLoadData();
                            while (reader.Read())  // Always call Read before accessing data.
                            {
                                int fldCntr = 0;
                                while (fldCntr < totFldCnt)
                                {
                                    // SDE.ST_ASTEXT(SHAPE)  //RETURNS oracle.manageddataacess.types.oracleclob CLOB
                                    // add clob instead of text to table
                                    var clob = reader.GetOracleValue(fldCntr) as OracleClob;
                                    if (clob != null)
                                    {
                                        //Console.WriteLine(reader.GetName(fldCnt2) + " " + clob.ToString() + " " + reader.GetValue(fldCnt2) + " " + rowCnt);
                                        newRow[fldCntr] = clob; //reader.GetValue(fldCntr);
                                    }
                                    else
                                    {
                                        //Console.WriteLine(reader.GetName(fldCnt2) + " " + reader.GetOracleValue(fldCnt2) + " " + reader.GetValue(fldCnt2) + " " + rowCnt);
                                        newRow[fldCntr] = reader.GetValue(fldCntr);
                                        //newRow[colCounter] = dataReader.GetValue(colCounter) is DBNull ? string.Empty : dataReader.GetValue(colCounter);                                   
                                        //newRow[colCounter]= objCol.GetValue[colCounter];
                                    }
                                    fldCntr = fldCntr + 1;
                                }
                            ...
                            dtab.EndLoadData();
                            _actrecCount.SelectString = rowCntr.ToString();
                        }
                        ...
                        else
                        {
                            lock (_aLock)
                            {
                                QueryText.SelectString = queryStr;
                                Results = dtab.DefaultView;
                            }
                            lock (_aLock)
                            {
                                BtnMapVis = true;
                            ...
                            _strMessage.SelectString = "Data returned.";
                        }
            ...
            {
                cpd.Message = "The sql statement was not able to execute. " + ex.Message.ToString() + " Please review the sql statement.";
                lock (_aLock)
                {
                    BtnMapVis = false;
                }
        ...
        public async Task AddToMapCallback(CancelableProgressorSource cpd)
        {
            try
            {
                await QueuedTask.Run(() =>
                {
                    ...
                    using (Database db = new Database(connectionProperties))
                    {
                        ...
                        if (_spatialCol.SelectString != "" && _objidCol.SelectString != "none")
                        {
                            ...
                            _actrecCount.SelectString = featureClass.GetCount().ToString();
                        ...
                        else
                        {
                            ...
                            _actrecCount.SelectString = pFL.GetTable().GetCount().ToString();
            ...
            try
            {
                await QueuedTask.Run(() =>
                {
                    ...
                    using (Database db = new Database(connectionProperties))
                    {
                        ...
                        //qds.SetShapeType(GeometryType.Point);
                        //qds.SetSpatialReference;
                        //qds.SetSRID

                        Table pTab2 = db.OpenTable(qds);
                        pTab2.GetDataConnection();
        ...
        public async Task AddToTOCCallbackOld(CancelableProgressorSource cpd)
        {
            try
            {
                await QueuedTask.Run(() =>
                {
                    ...
                    if (objfieldcount > 1)
                    {
                        ...
                        objForSet = fldLst.ToString().Split(',')[0];
        ...
        private string _heading = "Build Select Statememnt";
