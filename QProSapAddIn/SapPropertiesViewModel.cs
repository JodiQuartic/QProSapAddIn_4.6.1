...
{
    internal class SapPropertiesViewModel : Page
    {
        ...
        protected override Task InitializeAsync()
        {
            try
            {
                List<ConnectionItem> conitem = SapConfigModule.Current.ConnectionItems;
                Connections = conitem;
                Dictionary<string, string> settings = SapConfigModule.Current.Settings;
                    ...
                    using (FileStream fStream = new FileStream(contentPath, FileMode.Open))
                    {
                        if (fStream.CanRead)
                        {
                            ...
                            catch (Exception)
                            {
                                ...
                                System.Windows.MessageBox.Show("Could not get a stored password for Sap\r\n Please set it in Project Settings <<  Options");
                                return Task.FromResult(0); ;
                            }
